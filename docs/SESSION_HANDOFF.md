...
Dependencies: S022, S028, S033, and S040 are DONE; repository implementation requires no founder decision. PRs #437 and #438 repaired runtime state and credential-role mapping. Live authenticated deployment evidence now requires the stable staging backend's branch-scoped `SUPABASE_URL` repair, then a rerun with the rotated Beta owner and isolated field credentials.

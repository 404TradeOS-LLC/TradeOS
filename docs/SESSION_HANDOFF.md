...
- S012 (Job lifecycle normalization) is `READY` through this separate governance-only promotion. The current runtime already uses the canonical eight statuses and named service actions. A readiness audit found one documentation drift requiring explicit preservation: `JobsService.complete()` permits only `on_site -> completed`, despite older workflow text listing `traveling|paused -> completed`; S012 must not silently broaden that behavior. Its implementation must revalidate runtime drift before editing and must fail closed on any need for schema, status-vocabulary, RBAC/RLS, automatic-invoice, or broader architecture decisions. See `docs/architecture/S012_JOB_LIFECYCLE_PLAN.md`.

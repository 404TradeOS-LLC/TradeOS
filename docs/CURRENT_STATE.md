...
- Invoice line-item storage uses canonical selling-price columns `unit_price` and `line_total`; synchronized legacy `unit_cost`/`line_cost` aliases still remain in production during the expand/contract rollout. The tracked contraction has passed disposable PostgreSQL rehearsal with canonical data, index/constraint, and forced-RLS assertions, but remains pending the protected production approval/deployment gate; the `unitPrice`/`lineTotal` API contract stays unchanged.

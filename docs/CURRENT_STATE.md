...
- Team & Time staging integration: `/team-time` uses the existing signed-in Supabase session through a same-origin server route to call the staging `team-time` Edge Function. It supports assigned-job punches, breaks, manager review/correction, employee vs. subcontractor classification, and approved-hours CSV handoff. The route and navigation are disabled unless the server-only `TEAM_TIME_ENABLED=true` flag is set and `TEAM_TIME_SUPABASE_PROJECT_REF` exactly matches `NEXT_PUBLIC_SUPABASE_URL`; the feature is always disabled on Vercel Production. Payroll submission, location verification, and a live signed-in phone-to-office test are not implemented or claimed.

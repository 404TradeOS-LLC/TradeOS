...
Last reconciled against current `main` on 2026-08-14 for merged Costbook equipment and Athena foundation evidence. C004 equipment catalog foundation from PR #183 is merged; the `/costbook/equipment` frontend is now being hardened in PR #203 with a bounded 15-second load timeout and form/edit transitions locked while save/delete mutations are pending. Runtime implementation claims remain grounded in the code paths and merged evidence named below. Repository state does not by itself prove production deployment state, which must be verified through the approval-gated deployment workflows and the target platform.
...
- Costbook equipment catalog foundation (C004): organization-scoped equipment list/detail/create/update/delete under `/api/v1/costbook/equipment`, legacy equipment-route permission alignment, owner/admin-managed forced-RLS writes, cent-safe hourly-cost derivation, nullable daily-rate clearing, and `/costbook/equipment` with real-data loading/error/empty/read-only/mutation states. PR #203 adds bounded 15-second backend loading and locks editable form state/transitions while save or delete mutations are pending.

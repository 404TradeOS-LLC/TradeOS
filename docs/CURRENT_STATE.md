...
- Invoice line-item storage now uses canonical selling-price columns `unit_price` and `line_total`; the contract migration removed the temporary synchronized `unit_cost`/`line_cost` aliases after rollout verification, while the `unitPrice`/`lineTotal` API contract remains unchanged.

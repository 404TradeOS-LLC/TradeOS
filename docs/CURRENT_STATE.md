...
The estimate builder now embeds a contractor-focused Athena copilot that turns rough scope into a structured draft with extracted quantities, preparation, visible assumptions, Costbook/assembly matches, and a selected-item cost review. Clarification is presented one question at a time with quick choices and free-text input. Contractors select suggested lines and explicitly add them; the existing `/ai-estimator/draft` and `/ai-estimator/apply` contracts retain backend review-token, draft-status, organization-target, idempotency, and Estimate Engine safeguards. Unknown or ambiguous item counts remain unresolved without an apply token. Generated customer scope stays faithful to the contractor's input and does not infer exclusions. The visible 600 SF assumption applies only to explicitly described 2.5-car garage floor work; it is a heuristic, not a measured takeoff or verified price. The frontend includes saved and selected item costs; after application, Estimate Engine recalculates authoritative overhead, markup or margin, tax, and total and the refreshed estimate displays those persisted totals. Pre-apply sell-price projection is a follow-up and is not part of S053 acceptance. Authenticated browser certification at 1440/768/390 remains pending.

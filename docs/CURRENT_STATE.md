...
- `app/modules/costbook/candidateCostItem.ts` adds the first governed contract for a researched candidate cost item — a Zod schema capturing source/date/region/confidence plus an explicit `reviewStatus` (`candidate -> needs-review -> approved | rejected`) — and `isEligibleForCostbookPromotion()`, the single gate a future ingestion service must call before writing an approved candidate into `CostItem`/`Material`/`LaborRate`. This is a types/validation contract only: no route, service, or Prisma model exists yet, no candidate can reach "approved" without a named human `reviewedBy`/`reviewedAt`, and no production Costbook record or pricing value changed. See `docs/architecture/COSTBOOK_RESEARCH_INGESTION_DESIGN.md` for the intended end-to-end pipeline this is the first slice of.
- No migration was required; no Knowledge Engine cost/labor/equipment/material value changed; the Knowledge Engine still has an independent static corpus after this slice (it is not yet re-derived from governed Costbook data — that remains the target architecture, not current behavior).

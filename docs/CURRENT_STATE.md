...
Last reconciled against origin/main commit ca042c3a282b03d26f5f5fa389b7b49b9aa02e85 on 2026-08-27 after S043 implementation merge and completion reconciliation.
...
- The organization invoice queue now treats persisted `paid` as authoritative for `balanceDue: 0`, even when the supported manual mark-paid path intentionally creates no Payment row; `paidAmount` remains the recorded-payment aggregate and no ledger history is fabricated. This keeps queue and invoice-detail presentation consistent without changing S011 payment reconciliation or adding payment-entry UI.
...
- **S020 decision:** ADR-007 remains authoritative for legal-signature posture: TradeOS must not claim certificate-backed signing, identity verification, notarized execution, or standalone legal e-signature semantics. ADR-010 separately authorizes a customer-scoped magic-link identity for the public portal and customer attribution of a pending contract signature.
- **S020 completion:** S020 — Portal contract signing flow is `DONE` after implementation PR #322 merged on 2026-08-24 as `a3e9d376ebb1f350330b8924951c12ffc00911f3`; separate completion evidence is recorded in [S020_COMPLETION_EVIDENCE.md](architecture/S020_COMPLETION_EVIDENCE.md). Staff sign/void writes require organization scope and expected-status predicates, stale competing mutations fail closed before recording a losing event, and repeated voiding is rejected. ADR-010 adds the separately scoped public customer signing path without changing the legal-signature posture.
- **ADR-010 implementation completion:** The approved customer magic-link portal is complete after implementation PR #402 merged on 2026-08-28 as `9adb89e59e259adda037c9851657d0ea9f337a74`; completion evidence is recorded in [ADR-010_COMPLETION_EVIDENCE.md](architecture/ADR-010_COMPLETION_EVIDENCE.md). The public `/customer-portal/*` surface uses one-time hashed access tokens, short-lived hashed sessions, customer/tenant-scoped forced-RLS reads, replay and revocation protection, and an exact pending-contract customer signing policy with explicit portal attribution. The authenticated `/portal/*` staff preview remains separate. Hosted Verify #1766 and the required migration, dependency, docs, governance, live-documentation, and branch-currency checks passed. Authenticated rendered-browser/deployment verification remains an external follow-up; no beta-readiness claim is made.
...
- Supplier review queue and scheduler plumbing, including the S038 bounded
  background-attempt outcome contract and org/supplier transaction lock
...
- S038 background and retry semantics are implemented over existing async seams. Subscriber attempts now carry the event correlation ID, a stable event/subscriber idempotency key, and a one-based attempt number. Supplier scheduler outcomes use safe bounded failure codes and correlation metadata, and one organization/supplier feed transaction takes an advisory lock to prevent concurrent scheduler ticks from racing pending-proposal reconciliation. Observability export failures likewise return safe reason codes. Existing background sessions remain membership-derived and RLS-scoped. See [architecture/S038_BACKGROUND_RETRY_SEMANTICS.md](architecture/S038_BACKGROUND_RETRY_SEMANTICS.md); production scheduler configuration and live failure rehearsal remain external evidence.
...
- Customer portal has two explicit surfaces: `/portal/*` remains the authenticated staff preview, while `/customer-portal/*` uses the ADR-010 customer-scoped magic-link principal. Access tokens are one-time hashed records redeemed into short-lived hashed sessions; public reads are customer/tenant scoped and customer-originated writes are limited to the exact pending-contract signing transition. The public route does not claim certificate-backed or legally verified identity.
...
- S030 is DONE after implementation PR #341 merged as `d8e07606737de561b7cbed4e0be72ce875fae73c`; the Dispatcher Workspace verification preserves canonical Job actions, organization authorization, forced RLS, and the governed declined-assignment visibility migration. Repository verification passed; authenticated rendered browser evidence remains environment-dependent and was not fabricated. See `docs/architecture/S030_COMPLETION_EVIDENCE.md`.
- S042 is DONE after readiness PR #353 and implementation PR #354 merged as `8415c913cd0e596e0e2e18e33a8536f7257b6769`; completion evidence is recorded in `docs/architecture/S042_COMPLETION_EVIDENCE.md`. Refresh-token rotation is conditionally single-use under concurrency; local refresh sessions are revoked on logout, password-reset confirmation, and inactive-account rejection; Supabase JWTs require finite `exp`/`iat`; and project storage server actions fail closed before storage access when no server-side session exists. No schema, provider, role, permission, RLS redesign, or immediate access-token revocation model was introduced.

- S043 is DONE after implementation PR #395 merged as `ca042c3a282b03d26f5f5fa389b7b49b9aa02e85`; completion evidence is recorded in `docs/architecture/S043_COMPLETION_EVIDENCE.md`. Its bounded contract extends the existing Athena audit store with fixed, safe, correlated security events for authenticated requests, security decisions, privilege and tenant-boundary denials, and sensitive action attempts/completions, plus an owner/admin query surface behind existing observability authorization. No new store, schema, role, permission, RLS policy, retention policy, provider, or authentication model was introduced.

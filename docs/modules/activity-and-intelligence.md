...
- the embedded Athena estimating copilot exposes parsed scope, one-question clarification, and explicit add-to-estimate review through the existing estimator routes; these frontend and response-shape changes add no activity event type or intelligence write path. Existing redacted draft-generation metadata and reviewed-apply provenance remain the activity boundary.

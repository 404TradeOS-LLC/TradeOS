...
| Jobs and commercial records | `jobs`, `job_assignments`, `job_equipment`, `invoices`, `invoice_line_items`, `payments`, `service_agreements`, `contracts`, `proposal_deliveries`, `invoice_deliveries`, `contract_events` | Organization-scoped policy migrations and service-boundary authorization |
| Activity and collaboration | `activity_events`, `notifications`, `attachments`, `comments`, `tags`, `tag_assignments`, `saved_views`, `recent_items` | Intelligence foundation and organization-RLS policy migrations; actor/organization predicates are documented at the service boundary |
| Athena execution and audit | `athena_executions`, `athena_execution_transitions`, `athena_telemetry_records`, `athena_generation_runs`, `athena_generation_reviews`, `athena_memories`, `athena_events`, `athena_event_deliveries`, `athena_event_dead_letters`, `athena_alerts`, `athena_approvals`, `athena_audit_events` | Athena migrations explicitly enable and force RLS; policies use organization and actor context as applicable |
...
- The request session now stores canonical SQL roles while retaining raw legacy
  role compatibility in AuthContext. Cross-organization access continues to be

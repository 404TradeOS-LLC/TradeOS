...
`GET /api/v1/costbook/assemblies/starter-catalog` requires `costbook.read` and returns the TradeOS-authored residential recipe catalog with NAHB work groups, CSI codes, measurement bases, review guidance, and component slots. It contains no prices. `POST /api/v1/costbook/assemblies/starter-catalog/install` requires `costbook.write`; its strict body contains `templateId` and a `componentMappings` array of `{ componentKey, costItemId }`. Every required slot must appear exactly once and every Cost Item must be active and owned by the authenticated organization. The route creates an ordinary tenant-scoped reusable Assembly and AssemblyItem set through the existing service and request transaction. Duplicate assembly codes return `409`; missing/unknown mappings return `400`; unavailable or cross-tenant Cost Items return `404`.

...
Project Athena A12 business tools (`app/modules/athena-tools/**`) add no new REST routes under `/api/v1/estimates` or `/api/v1/jobs` - they are invoked only through the existing Athena kernel chat endpoint (`POST /api/v1/athena/chat`, dark behind `ATHENA_KERNEL_ENABLED`), calling `EstimateEngineService`/`JobsService` directly rather than over HTTP. `EstimateEngineService` gained one new read-only method, `compareEstimates()` (no route), and `EstimateEngineService.create()`/`finalize()` plus `JobsService.schedule()`/`addAssignment()`/`complete()` gained an additive, optional `athenaEvent` field on their existing return shapes (a real A8 event reference when publish succeeds) - existing route responses are unaffected since JSON serialization of an extra optional field is backward compatible. See [athena/roadmap/A12-business-tool-rollout-implementation-plan.md](athena/roadmap/A12-business-tool-rollout-implementation-plan.md).

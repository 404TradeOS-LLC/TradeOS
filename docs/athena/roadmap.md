...
| A1 AI kernel | Runtime shell, model adapter seam, prompt boundary, lifecycle state, minimal telemetry | A0 plus A0.5 corrections | Kernel service skeleton, no business tools, no broad context, no memory writes | Can accept request and return no-op/draft response with persisted execution state | unit tests, lifecycle tests, cancellation/timeout tests, trace assertions, `athena:contracts`, `athena:smoke` when available | Feature flag off returns legacy UI |
...
| A3 Context engine | Immutable context assembly with existing app-backed providers first | A1, A2 | Provider contracts, selected scope, sensitivity, freshness metadata, degraded context | Missing provider degrades without fabricating facts or widening scope | provider tests, cache tests, PII/redaction tests, tenant cache tests, `athena:perf` when available | Fall back to minimal request/user context |
...
| A10 Observability | Dashboards, exporters, alerts, retention, cost reporting, audit maturity | A1-A9 minimal telemetry | Operational observability suite over C011 records | Requests and actions are traceable in operator workflows | smoke tests, redaction tests, incident drills | Disable non-critical sampling/export |
...
- Do not exit A1 without lifecycle state, cancellation/timeout behavior,
  minimal trace/cost telemetry, and named Athena validation gates documented.
- Do not load all context sections eagerly; providers activate by intent,
  selected scope, permissions, freshness, and budget.

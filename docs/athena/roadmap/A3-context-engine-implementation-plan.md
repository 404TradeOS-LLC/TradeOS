...
- A new `app/modules/athena-context-engine/` module implementing:
  ...
  - A context assembler that activates providers by mode
    ...
    item/byte budgets (`maxItems`, `maxBytes`), enforces
    ...
    treats `criticality: "critical"` as a hard stop condition, and degrades
    or omits per each non-critical provider's declared `failureBehavior`
...
- Exactly two first-party providers, both backed by data the app already
  ...
  2. **`dispatch` provider** - actor-scoped job list, reusing the existing
     ...
     `JobsService.list()` inside a properly-scoped database session for
     overview context, or `JobsService.getById()` when `selectedScope.jobId`
     is present so a job-scoped request never receives an unrelated broader
     page of jobs. RLS narrows the result set by role without the provider
     adding its own duplicate actor filter. `sensitivity: "internal"`,
     `activation: "lazy_intent"`, `freshness.status` computed from the read
     (`live` for a direct read, `fresh` for a cache hit, never fabricated).
...
- Selected-scope handling: `AthenaSelectedScope` (already defined in A1's
  types) is read as a narrowing input by the `dispatch` provider. `jobId`
  takes precedence over broader `projectId`/`customerId` filters, and
  selected scope is never treated as proof of access by itself.
- Degraded/partial context behavior: a non-critical provider failure
  produces a `status: "degraded"` or `"omitted"` section plus a warning,
  never a stopped assembly; a critical provider failure (none of A3's two
  providers are marked critical) stops dependent planning and is exercised by
  a test even though A3 has no real critical provider yet.
...
| Assembler | `app/modules/athena-context-engine/assembler.ts` | Activates providers by mode, enforces per-provider item/byte budgets, enforces timeout via a dispatcher-owned deadline, computes freshness, applies failure behavior, never fabricates or widens scope |
...
- A section (e.g. `"dispatch"`) may have at most one *active* provider
  registered at a time in A3, including multiple versions of the same
  provider ID - multiple competing providers for the same section is
  deferred until there's a real reason for one (e.g. a plugin provider in
  A13).
- A provider with `criticality: "critical"` must use
  `failureBehavior: "stop"`; contradictory critical/degrade or critical/omit
  definitions are rejected at registration.
...
- `knowledgeEngine` may use `cacheKeyPolicy: "tenant_actor_permission_input"`
  ...
  in the A0.5 review.
- The cache-key builder must include organization, actor, actor role,
  effective permission snapshot, provider version, and scoped input - a cache
  entry built under one org/actor/role/permission combination must never be
  returned for another, proven by dedicated tests.
...
- `AthenaSelectedScope` (already defined) is the only scope-narrowing input
  ...
  (`jobs_select_policy`) and `JobsService` lookups, not from trusting
  `selectedScope.jobId`.
...
- Non-critical provider failure (`failureBehavior: "degrade"` or
  `"omit"`) produces a `status: "degraded"` or `"omitted"` section and
  a warning; assembly continues with the rest of the context.
- Critical provider failure (`criticality: "critical"` and therefore
  `failureBehavior: "stop"`) stops assembly for the whole context, matching
  the runtime doctrine ("Critical provider failure stops planning for
  dependent actions"). Neither A3 provider is marked critical, so this path
  is exercised only by a fixture provider in tests.
- No provider failure may ever cause the assembler to fabricate a fact,
  ...
  `status: "degraded"`, `"omitted"`, or `"denied"`, never invented data.
...
- A code-defined context provider registry exists with `register()`/
  ...
  narrowed from C010.
- The assembler enforces activation mode, per-provider item/byte budgets,
...
- A tenant-qualified cache-key builder exists and passes two-organization,
  actor-role, effective-permission, provider-version, and scoped-input
  isolation tests.

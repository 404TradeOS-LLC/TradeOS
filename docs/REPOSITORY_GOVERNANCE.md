...
Supplemental workflows such as `Sprint governance`, `Migration safety`, `PR branch currency`, `Live documentation reconciliation`, `Merge readiness`, authenticated browser evidence, nightly full regression, and workflow-health reporting provide earlier or richer evidence. The sprint-governance docs-only guard applies when a diff adds `Status: READY`; moving a sprint from `PLANNED` to `IN_REVIEW` may accompany implementation work and does not waive its acceptance or verification gates. A bounded customer-match lookup that returns the maximum result count is treated as incomplete and blocks creation until the user retries; truncation is not evidence that no match exists. Their presence in the repository does not make them required branch-protection checks or grant merge authority; only live GitHub ruleset configuration can do that. `docs/CI_ACCELERATION.md` documents their operating intent and credentials/evidence boundaries.

...
- S021 readiness was promoted through a governance-only lane, and its isolated implementation lane is now validating the presentation contract. Existing invoice amount, recorded-payment aggregation, derived balance/partial/overdue semantics, billing authorization, authenticated portal reads, and forced PostgreSQL RLS remain the source of truth. S021 remains presentation-centric; new payment processing, public payment links, ledger/money semantics, schema, auth, or RLS/RBAC changes are forbidden. S020 remains planned pending founder/legal-signature semantics. See `docs/architecture/S021_PORTAL_INVOICE_PRESENTATION_PLAN.md`.

## Active engineering queue

Prioritize existing authorized work before inventing new scope. S007-S012, S018, and S019 are complete with merged implementation and completion evidence. S021 is now `READY` for the next single numbered implementation lane; S020 remains planned pending founder/legal-signature semantics; S027 remains separately blocked on authenticated rendered Costbook browser evidence.

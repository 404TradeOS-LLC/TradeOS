...
| TD-002 | Repository structure | ~~`packages/knowledge-engine/knowledge-engine/**` was a prohibited self-nested package duplicate.~~ Founder-approved cleanup completed on 2026-08-24 after a fresh full re-hash, repo-wide reference grep, and loader/path-resolution proof found no unique runtime/build dependency or duplicate-only files. | `packages/knowledge-engine/PATHS.md`, `packages/knowledge-engine/path-manifest.json`, cleanup branch evidence | HIGH | Resolved — the former duplicate tree is removed and the canonical top-level package remains authoritative. | Landed in the governed cleanup PR; no further action required for TD-002. | `packages/knowledge-engine/knowledge-engine/**` | Large | ALREADY_RESOLVED |
...
6. ~~Handle TD-002 only after the remaining destructive-cleanup proof gates are complete.~~ Done — founder-approved duplicate-tree cleanup proof gates passed on 2026-08-24; the canonical package tree remains intact.

...
Governance note (2026-08-11): with the earlier PR-overlap claim now stale, the next directly commissioned Costbook implementation slice is C004 Equipment Catalog Foundation on a dedicated branch from current `main`. That bounded slice extends the existing authenticated Costbook workspace/material/labor seams with organization-scoped equipment CRUD, permissions, UI, tests, and factual docs only; it does not promote S027 to `READY`.

...
S011 invoice lifecycle normalization is now `IN_REVIEW` on the separate implementation branch after its governance-only readiness promotion merged. The approved bounded implementation owns backend payment reconciliation correctness: serialize concurrent reconciliation per Invoice, aggregate valid recorded payments with cent-safe semantics, transition eligible fully covered `sent` invoices to persisted `paid` inside the existing request-scoped transaction/event boundary, and exclude persisted terminal invoices from unpaid/partially-paid/overdue follow-up queues. Overdue and partially-paid remain derived; payment-entry UI expansion is deferred; persisted `partially_paid`, a new persisted-overdue writer, `viewed` tracking, billing/payment-processor or portal redesign, unrelated Invoice mutation repairs, and S012 remain out of scope. S011 is not `DONE` until the implementation PR merges and separate completion evidence lands.

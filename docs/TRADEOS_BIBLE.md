...
S018 — Customer portal authentication hardening is now `READY`: dependencies S007, S009, S010, and S011 are complete, and readiness reconciliation found no overlapping live work. The current portal uses the existing authenticated Supabase session and protected bearer-authenticated API/RLS boundary; there is no separate customer token or identity model. The authorized implementation is to harden and prove invalid/expired/revoked-session handling, active-membership resolution, same-organization resource authorization, cross-organization denial, existing mutation permissions, and forced PostgreSQL RLS while preserving the current route/API and request-session architecture. A new customer-auth product, public tokenized links, auth-provider replacement, RBAC/RLS redesign, or portal redesign requires a separate founder decision and is forbidden in S018. See `docs/architecture/S018_CUSTOMER_PORTAL_AUTHENTICATION_PLAN.md`.

...
composite benchmark API contract, de-duplicates by source/year/item, reports
source rows that violate the benchmark contract, and posts bounded idempotent
...
def number(value: Any) -> float | None:
    if value is None or value == "":
        return None
    result = float(value) if isinstance(value, (int, float)) else float(str(value).replace("$", "").replace(",", "").strip())
...
def normalize_review_status(value: Any) -> str:
    ...
    aliases = {"candidate-review": "needs-review", "candidate review": "needs-review", "pending": "needs-review", "approved": "reviewed", "ready": "reference"}
    text = aliases.get(text, text)
    if text.startswith("review:"):
        return "needs-review"
    return text if text in {"reference", "needs-review", "reviewed", "rejected"} else "reference"
...
def normalize_row(row: list[Any], mapping: dict[str, int], default_source_file: str, fallback_row: int) -> dict[str, Any] | None:
    ...
    if low is not None and weighted < low:
        raise ValueError(f"weighted average {weighted} < low {low} for {year}/{item_code}")
    if high is not None and weighted > high:
        raise ValueError(f"weighted average {weighted} > high {high} for {year}/{item_code}")
    ...
    result: dict[str, Any] = {
        ...
        "sourceRow": integer(get(row, mapping, "source_row")) or fallback_row,
        ...
        "priceBasis": str(get(row, mapping, "price_basis") or DEFAULT_PRICE_BASIS).strip(),
...
def load_rows(path: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    with zipfile.ZipFile(path) as zf:
        ...
        rejected_by_key: dict[tuple[str, str], dict[str, Any]] = {}
        for sheet_name in ("Price_History", "Costbook_Import"):
            ...
            for excel_row, row in enumerate(rows[header_index + 1 :], start=header_index + 2):
                item_hint = str(get(row, mapping, "item_code") or "").strip()
                try:
                    normalized = normalize_row(row, mapping, path.name, excel_row)
                except (TypeError, ValueError) as exc:
                    if item_hint:
                        rejected_by_key[(sheet_name, item_hint)] = {
                            "sheet": sheet_name,
                            "row": excel_row,
                            "itemCode": item_hint,
                            "reason": str(exc),
        ...
        # If a source/year/item is invalid in either prepared sheet, reject that
        # benchmark identity rather than silently importing a conflicting copy.
        rejected_items = {entry["itemCode"] for entry in rejected_by_key.values()}
        deduped = {key: row for key, row in deduped.items() if row["itemCode"] not in rejected_items}
    rows = sorted(deduped.values(), key=lambda r: (r["sourceYear"], r["itemCode"]))
    rejections = sorted(rejected_by_key.values(), key=lambda r: (r["itemCode"], r["sheet"], r["row"]))
...
def main() -> int:
    ...
    rows, rejections = load_rows(args.input)
    if not rows:
        raise SystemExit("No importable INDOT rows found")
    years = sorted({row["sourceYear"] for row in rows})
    print(f"Validated {len(rows)} importable INDOT source/year/item rows for years {years}; rejected={len(rejections)}")
    for rejection in rejections:
        print(f"REJECT {rejection['sheet']} row {rejection['row']} {rejection['itemCode']}: {rejection['reason']}")
    if args.json_out:
        args.json_out.write_text(json.dumps({"rowCount": len(rows), "years": years, "rejectedCount": len(rejections), "rejections": rejections, "rows": rows}, indent=2) + "\n", encoding="utf-8")
    ...
    print(f"Import complete: batches={batch_count} received={total_received} upserted={total_upserted} rejected={len(rejections)}")
    return 0

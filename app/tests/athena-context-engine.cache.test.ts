...
describe("athena context cache", () => {
  describe("buildAthenaContextCacheKey", () => {
    it("produces the same key for the same tenant/actor/permissions/provider/input", () => {
      const input = { orgId: "org-1", actorUserId: "user-1", actorRole: "owner", effectivePermissions: ["crm.read"], providerId: "p1", providerVersion: "1.0.0", scopedInput: { jobId: "job-1" } };
      expect(buildAthenaContextCacheKey(input)).toBe(buildAthenaContextCacheKey({ ...input }));
    });

    it("is insensitive to permission array order", () => {
      const base = { orgId: "org-1", actorUserId: "user-1", actorRole: "owner", providerId: "p1", providerVersion: "1.0.0", scopedInput: {} };
      const keyA = buildAthenaContextCacheKey({ ...base, effectivePermissions: ["crm.read", "billing.read"] });
      const keyB = buildAthenaContextCacheKey({ ...base, effectivePermissions: ["billing.read", "crm.read"] });
      expect(keyA).toBe(keyB);
    });

    it("produces a different key for a different organization", () => {
      const base = { actorUserId: "user-1", actorRole: "owner", effectivePermissions: [], providerId: "p1", providerVersion: "1.0.0", scopedInput: {} };
    ...
    it("produces a different key for a different actor", () => {
      const base = { orgId: "org-1", actorRole: "owner", effectivePermissions: [], providerId: "p1", providerVersion: "1.0.0", scopedInput: {} };
      expect(buildAthenaContextCacheKey({ ...base, actorUserId: "user-a" })).not.toBe(buildAthenaContextCacheKey({ ...base, actorUserId: "user-b" }));
    });

    it("produces a different key for a different role even when actor and permissions are the same", () => {
      const base = { orgId: "org-1", actorUserId: "user-1", effectivePermissions: ["crm.read"], providerId: "p1", providerVersion: "1.0.0", scopedInput: {} };
      expect(buildAthenaContextCacheKey({ ...base, actorRole: "owner" })).not.toBe(buildAthenaContextCacheKey({ ...base, actorRole: "technician" }));
    });

    it("produces a different key for a different permission snapshot", () => {
      const base = { orgId: "org-1", actorUserId: "user-1", actorRole: "owner", providerId: "p1", providerVersion: "1.0.0", scopedInput: {} };
      expect(buildAthenaContextCacheKey({ ...base, effectivePermissions: ["crm.read"] })).not.toBe(buildAthenaContextCacheKey({ ...base, effectivePermissions: ["billing.write"] }));
    });

    it("produces a different key for a different provider version", () => {
      const base = { orgId: "org-1", actorUserId: "user-1", actorRole: "owner", effectivePermissions: [], providerId: "p1", scopedInput: {} };
    ...
    it("produces a different key for different scoped input", () => {
      const base = { orgId: "org-1", actorUserId: "user-1", actorRole: "owner", effectivePermissions: [], providerId: "p1", providerVersion: "1.0.0" };

...
describe("Stripe webhook verification", () => {
  ...
  it("rejects a signed event envelope without data.object", () => {
    const body = Buffer.from(JSON.stringify({ id: "evt_tradeos_3", type: "invoice.paid" }));
    const timestamp = Math.floor(Date.now() / 1000);
    const signature = crypto
      .createHmac("sha256", process.env.STRIPE_WEBHOOK_SECRET!)
      .update(`${timestamp}.${body.toString("utf8")}`)
    ...
    expect(() => verifyStripeWebhook(body, `t=${timestamp},v1=${signature}`)).toThrow("Invalid Stripe event envelope");
  });
});

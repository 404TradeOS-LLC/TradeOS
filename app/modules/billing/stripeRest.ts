...
export async function stripeRequest<T>(
  ...
  try {
    ...
    if (error instanceof Error && (error.name === "AbortError" || error.name === "TimeoutError")) {
      throw new ApiError(504, "Stripe request timed out");
    }
    throw new ApiError(502, "Stripe request failed");
  }
...
export function verifyStripeWebhook(rawBody: Buffer, signatureHeader: string | undefined): StripeEventEnvelope {
  ...
  const event = parsed as { id?: unknown; type?: unknown; created?: unknown; data?: unknown };
  const data = event.data;
  const object = data && typeof data === "object" ? (data as { object?: unknown }).object : null;
  if (
    !parsed ||
    typeof parsed !== "object" ||
    typeof event.id !== "string" ||
    typeof event.type !== "string" ||
    (event.created !== undefined && (typeof event.created !== "number" || !Number.isFinite(event.created))) ||
    !object ||

...
export function getTradeOsAppUrl(): string {
  ...
  if (value) {
    ...
    try {
      ...
      throw new ApiError(503, "TradeOS application URL is invalid");
    }
    if (process.env.NODE_ENV === "production" && parsed.protocol !== "https:") {
      throw new ApiError(503, "TradeOS application URL must use HTTPS in production");
    }
    if (parsed.protocol !== "https:" && !(parsed.protocol === "http:" && ["localhost", "127.0.0.1", "::1"].includes(parsed.hostname))) {
      throw new ApiError(503, "TradeOS application URL must use HTTPS outside loopback development");
    }

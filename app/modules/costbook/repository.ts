...
export class CostbookRepository {
  ...
  async listEquipment(organizationId: string): Promise<CostbookEquipmentRecord[]> {
  ...
  async getEquipmentById(organizationId: string, id: string): Promise<CostbookEquipmentRecord | null> {
  ...
  async createEquipment(organizationId: string, input: CostbookEquipmentInput): Promise<CostbookEquipmentRecord> {
  ...
  async deleteEquipment(organizationId: string, id: string): Promise<boolean> {
    const existing = await prisma.equipment.findFirst({ where: { id, orgId: organizationId } });

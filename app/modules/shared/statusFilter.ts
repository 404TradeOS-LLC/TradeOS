 ...
 * still hold legacy raw values (proposal rows store "rejected" for canonical
 * "declined"; estimate rows may contain historical raw values) — a naive
 * `status: { in: canonicalStatuses }` filter would silently miss those rows.
 * Canonical values are always included as-is since new rows are written with
 * them directly.
 */

 ...
 * map (e.g. `legacyEstimateStatusMap`). Estimate/Proposal/Invoice rows can
 * still hold legacy raw values (estimate rows store "sent" for canonical
 * "ready"; proposal rows store "rejected" for canonical "declined") — a
 * naive `status: { in: canonicalStatuses } }` filter would silently miss
 * those rows. Canonical values are always included as-is since new rows are
 * written with them directly.
 */

...
async function main() {
  ...
  // 1. Deterministic URL resolution across the supported sources.
  ...
  // 2. The resolved URL must be an approved non-production host.
  const parsed = assertApprovedRcUrl(url);

  // 3. Environment identity. The "actual" side is derived from the deployment
  //    that was resolved, not copied from the operator's own input, so the
  //    assertion can actually fail when the declared environment is wrong.
  const derivedEnvironment = deriveEnvironmentFromUrl(parsed.origin);
  const environment = assertEnvironmentIdentity(expectedEnvironment, derivedEnvironment);
  if (actualEnvironment && actualEnvironment.trim().toLowerCase() !== derivedEnvironment) {
    throw new RcTargetError(
      `BETA_ACTUAL_ENVIRONMENT is "${actualEnvironment}" but the resolved deployment is "${derivedEnvironment}".`,
      "ENV_MISMATCH",
    );
  }
  ...
  const target = {
    ...
    environment,
    environmentSource: "derived-from-deployment-host",
  ...
  console.log(
    `Resolved RC target: ${target.baseUrl} (source: ${source}, environment: ${environment} derived from the deployment host)`,
  );

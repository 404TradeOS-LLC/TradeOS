...
const routes = (process.env.RC_ROUTES ||
  "/dashboard,/customers,/projects,/estimates,/proposals,/contracts,/jobs,/invoices,/costbook")
  .split(",")
...
try {
  ...
  for (const route of routes) {
    ...
    const passed = status > 0 && status < 400 && bodyTextLength > 0;
    ...
    if (!passed) throw new Error(`${route} failed smoke check: status=${status}, body=${bodyTextLength}.`);
  }
  await context.close();
} finally {
  await browser.close();

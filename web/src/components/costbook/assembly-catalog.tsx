...
export function AssemblyCatalog({ initialAssemblies, costItems, canWrite, canManage }: {
  ...
  const [selectedId, setSelectedId] = useState(initialAssemblies.find((item) => item.isActive)?.id ?? "");
  ...
  useEffect(() => {
    ...
    clientFetch<AssemblyItem[]>(`/costbook/assemblies/${selectedId}/items`)
      ...
      .catch((err) => { if (active) setError(err instanceof Error ? err.message : "Assembly components could not be loaded."); })
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(260px,0.8fr)_minmax(0,2fr)]">
    <aside className="grid content-start gap-4">
      ...
      </form> : <div className="rounded-lg border border-border/70 bg-surface p-4 text-sm text-muted-foreground">Read-only Costbook access. Assembly mutations are hidden.</div>}
      ...
      <div className="overflow-hidden rounded-lg border border-border/70 bg-surface">
        ...
        {assemblies.length === 0 ? <div className="p-4 text-sm text-muted-foreground">No active assemblies.</div> : <div className="divide-y divide-border/70">{assemblies.map((assembly) => <button key={assembly.id} type="button" onClick={() => setSelectedId(assembly.id)} className={`w-full px-4 py-3 text-left text-sm ${assembly.id === selectedId ? "bg-muted" : "hover:bg-muted/50"}`}><span className="block font-medium text-foreground">{assembly.name}</span><span className="font-mono text-xs text-muted-foreground">{assembly.code} · {assembly.unitOfMeasure}{assembly.isTemplate ? " · Template" : ""}</span></button>)}</div>}
      </div>
    </aside>
    ...
    <section className="grid content-start gap-4">
      ...
      {!selected ? <EmptyState title="Choose an assembly" description="Select an assembly to review its composition." /> : <>
        <div className="flex flex-col gap-3 rounded-lg border border-border/70 bg-surface p-4 sm:flex-row sm:items-start sm:justify-between">
          <div><p className="font-mono text-xs text-muted-foreground">{selected.code}</p><h1 className="text-xl font-semibold text-foreground">{selected.name}</h1><p className="mt-1 text-sm text-muted-foreground">{selected.description || "No description"} · {selected.unitOfMeasure}</p></div>
        ...
        {loadingItems ? <div className="rounded-lg border border-border/70 bg-surface p-6 text-sm text-muted-foreground">Loading components…</div> : items.length === 0 ? <EmptyState title="No components yet" description="Add active CostItems or child Assemblies to build this composition." /> : <div className="overflow-hidden rounded-lg border border-border/70 bg-surface"><div className="divide-y divide-border/70">{items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 p-4"><div><p className="font-medium text-foreground">{item.componentName}</p><p className="font-mono text-xs text-muted-foreground">{item.componentCode} · {item.componentType === "cost_item" ? "Cost item" : "Assembly"} · {item.quantityPerUnit} {item.componentUnitOfMeasure}</p></div>{canWrite ? <Button type="button" variant="ghost" size="sm" onClick={() => removeComponent(item.id)} disabled={saving}><Trash2 className="size-4" aria-hidden="true" />Remove</Button> : null}</div>)}</div></div>}
      </>}
    </section>
  </div>;
}

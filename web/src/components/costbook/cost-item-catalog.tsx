...
export function CostItemCatalog({ initialCostItems, subcategories, laborRates, materials, equipment, canWrite, canManage }: {
  ...
  return <div className="grid gap-6">
    {canWrite ? <section className="rounded-lg border border-border/70 bg-surface p-4" aria-label={editing ? "Edit cost item" : "Create cost item"}>
    ...
    </section> : <div className="rounded-lg border border-border/70 bg-surface p-4 text-sm text-muted-foreground">{canManage ? "You can deactivate Costbook items, but this role cannot create or edit them." : "You have read-only Costbook access. Cost-item create and edit controls are hidden for this role."}</div>}
    {error ? <p role="alert" className="text-sm text-destructive">{error}</p> : null}

    {items.length === 0 ? <EmptyState title="No active cost items yet" description={canWrite ? "Add a cost item to connect your hierarchy and estimating catalogs." : "No active cost items are available for this organization."} /> : <section className="overflow-hidden rounded-lg border border-border/70 bg-surface" aria-label="Cost item catalog">

    ...
    <Card className="border-border/70">
      <CardHeader className="border-b border-border/60">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <CardTitle>Tasks To Move</CardTitle>
            <CardDescription>Live project tasks across the organization, ordered by due pressure so the dashboard surfaces work that needs a decision now.</CardDescription>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge variant="outline">{snapshot.overdueCount} overdue</Badge>
            <Badge variant="outline">{snapshot.dueTodayCount} due today</Badge>
            <Badge variant="outline">{snapshot.blockedCount} blocked</Badge>
          </div>
        </div>
      </CardHeader>
      <CardContent className="grid gap-3">
        {errorMessage ? (
          ...
          snapshot.openTasks.slice(0, 6).map((task) => (
            <article key={task.id} className="rounded-xl border border-border/60 bg-muted/20 p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <p className="font-mono text-xs uppercase tracking-[0.18em] text-muted-foreground">{formatTaskDueLabel(task, now, timeZone)}</p>
                  </div>

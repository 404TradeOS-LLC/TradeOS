...
export default async function CostbookPriceHistoryPage({ searchParams }: { searchParams: Promise<HistoryQuery> }) {
  ...
  return <div className="flex flex-col gap-6">
    ...
    <CatalogQueryControls pathname="/costbook/price-history" query={{ limit: query.limit }} total={data.materialTotal} shown={history.materialChanges.length} nextCursor={data.materialNextCursor} cursorParam="materialCursor" sortOptions={[{ value: "createdAt", label: "Newest" }]} />
    ...
    <CatalogQueryControls pathname="/costbook/price-history" query={{ limit: query.limit }} total={data.estimateTotal} shown={history.estimateSnapshots.length} nextCursor={data.estimateNextCursor} cursorParam="estimateCursor" sortOptions={[{ value: "createdAt", label: "Newest" }]} />
    <section className="overflow-hidden rounded-lg border border-border/70 bg-surface">

...
export default async function CostbookCostItemsPage() {
  ...
  if (!token) {
    ...
    try {
      [workspace, costItems, subcategories, laborRates, materials, equipment] = await Promise.all([
        ...
        apiFetch<Subcategory[]>("/api/v1/costbook/subcategories", { token }),
        apiFetch<LaborRate[]>("/api/v1/costbook/labor-rates", { token }),
        apiFetch<Material[]>("/api/v1/costbook/materials", { token }),
        apiFetch<Equipment[]>("/api/v1/costbook/equipment", { token }),
      ]);
    } catch (error) {
            ...
            <div className="rounded-lg border border-border/70 bg-surface p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-2 font-mono text-3xl font-semibold tabular-nums text-foreground">{costItems.length}</p>
                </div>
